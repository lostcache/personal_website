---
title: "My Take on the 1🐝🏎️ Challenge"
description: "My attempt at the legendary 1🐝🏎️ challenge. (No CPUs were harmed in the making of this blog post. RAM? Maybe.)"
pubDate: "Dec 16 2025"
---

### Why?

All my life (read: since I was `{current_age_dec_2026 - 2}` years old), I’ve wanted a job where I could ignore business logic and just make things go _brrrrr_.
...
I’ve learned the hard way like, _really_ hard way—to start simple.
...
TODO:
1) process batches of file in parallel using threads.
...
For this version we've made sure there no unnecessay memory allocations and are using 2 threads per core.
...
Okay the program spends most of the time int **processLines()** function, **libc.so** and **std::map\<...\>::operator[]** takes up the majority and almost equal time share.

It's hard so prevent some *unnecessary* memory allocations because of the behavior of **std::map\<...\>()**.
...
It's far more efficient to use **O(1)** complexity operations provided by **std::unordered_map\<...\>()**. The only problem is that we need to print the output in lexicographical order of location names.

Thats cakewalk in our case as we're only dealing with at most 10,000 unique locations. We can just collect them in a **std::vector\<std::string\>()** and sort it to then use it to access elements fron the **std::unordered_map\<...\>()**.

And we can reserve memory for both of those data-structures!
