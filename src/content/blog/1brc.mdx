...
I’ve learned the hard way like ... _really_ _really_ hard way to start simple.
...
```sh
Time (mean ± σ):     34.195 s ±  0.732 s    [User: 101.743 s, System: 11.998 s]
Range (min … max):   33.147 s … 35.211 s    10 runs
```

**mmap** is a game changer without any exageration. Not only we get rid of the overhead because of reading from streams. We no longer have to allocate 512MB buffers to read from file. We directly use the mapped file and the code is sooo much simpler to read and understand! I love pointer arithmetic.

Since we're going for raw speed it is only fair to quote one of the fastest human beings and the flying dutchman.

**_That is just simply lovely!_**
-Max Verstappen.
