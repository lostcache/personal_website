---
title: "The 1🐝🏎️ Challenge"
description: "My attempt at the legendary 1🐝🏎️ challenge. (No RAM were harmed in the making of this blog post. CPU? Maybe.)"
pubDate: "Last Updated, Dec 29 2025"
---

import ExpandableCode from "../../components/ExpandableCode.astro";

### The Challenge 👉 [1🐝🏎️](https://github.com/gunnarmorling/1brc)
...
I’ve learned the hard way ... _really_ hard way to start simple.
...
The 2024 version of me would probably just throw standard optimizations at this and hope for the best. But we're doing science here. I want to know exactly _why_ it's taking so long and where the bottleneck lies as 2026 version of me prefers targeted efforts.
...
The biggest compoenets of runtime are **parseLine()**, **libc.so** and **std::map\<...\>::operator[]** which is what i expected as the code does a lot of parsing, memory allocation/freeing and map operations (a Billion times).
...
Let's look at the 🔥📊 to get a better idea.
...
Thats no biggie as we're only dealing with at most 10,000 unique locations. We can just collect them in a **std::vector\<std::string\>()** and sort it to then use it to access elements from the **std::unordered_map\<...\>()**.
...
It was actually kind stupid to not do this straight away 😅.

But 🔥📊 >> 🧠🤓. let's look what's going on.
...
```sh
Time (mean ± σ):     42.432 s ±  0.521 s    [User: 140.723 s, System: 14.162 s]
Range (min … max):   41.611 s … 43.481 s    10 runs
```
...
<ExpandableCode minHeight="300px">
...
```sh
Time (mean ± σ):     32.188 s ±  0.101 s    [User: 71.249 s, System: 11.052 s]
Range (min … max):   32.047 s … 32.406 s    10 runs
```

<object
  type="image/svg+xml"
  data="/final_probably.svg"
  style={{ width: "100%" }}
></object>

Nice.

I finally resort to open-addressed hashmap with very simple hash function and we're now doing slightly better than the most optimal solution.

TODO: I'd like to try some ideas to make FastMap a bit more cache friendly by using just one 32 bit integer to store both min and max values but i doubt it'll make much of a difference, still will probably definitely try.

TODO: Summarize current results nicely in a table with benchmarks.

But honestly i'm a little bored and would rather just jump to some rust, zig (my personal favorite) [blog](/blog/1brc_again).
